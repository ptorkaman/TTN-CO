...
namespace TTN
{
    ...
    public class LessThanOrEqual : Criteria
    {
        public override Expression GetExpression(ParameterExpression parameter)
        {
            ...
            //return firstdOperandType.FullName.StartsWith(typeof(Nullable<>).FullName) ? Expression.LessThanOrEqual(Expression.Convert(leftExpression, Nullable.GetUnderlyingType(firstdOperandType)), rightExpression) : Expression.LessThanOrEqual(leftExpression, rightExpression);
