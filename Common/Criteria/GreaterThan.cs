...
namespace TTN
{
    ...
    public class GreaterThan : Criteria
    {
        public override Expression GetExpression(ParameterExpression parameter)
        {
            ...
            //return firstdOperandType.FullName.StartsWith(typeof(Nullable<>).FullName) ? Expression.GreaterThan(Expression.Convert(leftExpression, Nullable.GetUnderlyingType(firstdOperandType)), rightExpression) : Expression.GreaterThan(leftExpression, rightExpression);
            return ExpressionHelper.CreateConditionalExpression(parameter, this.FirstOprand.ToString(), ObjectType, this.SecondOperand, this.SecondOperand.GetType(), new GreaterThanConditionExpressionBuilder());
        }
    }
}
