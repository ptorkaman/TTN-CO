...
namespace TTN
{
    public class TreeNode<T> : ICloneable<TreeNode<T>>
    {
        ...
        {
            IEnumerable<T> list = includeSelf ? new[] { Value } : Enumerable.Empty<T>();

            IEnumerable<T> result = list.Union(_children.SelectMany(x => x.Flatten()));
            if (expression != null)
            {
        ...
        {
            IEnumerable<TreeNode<T>> list = includeSelf ? new[] { this } : Enumerable.Empty<TreeNode<T>>();
